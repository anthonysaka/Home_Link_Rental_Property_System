...
CREATE PROCEDURE sp_createUser (in pa_username VARCHAR(15), in pa_email VARCHAR(50), in pa_password VARCHAR(25),
		sp_search_publication_by_dir						in pa_name VARCHAR(20), in pa_lastname VARCHAR(30), in pa_gender VARCHAR(15),
...
BEGIN
	SELECT * FROM t_property WHERE address = pa_direccion AND status_property = "Disponible";
END##
DELIMITER ;
